...
// Handle prediction request (SVM model prediction)
app.post('/predict', (req, res) => {
    const inputData = {
        feelslikenow: req.body.feelslikenow,
        precipcover: req.body.precipcover,
        sealevelpressure: req.body.sealevelpressure,
        surrounds: req.body.surrounds,
        years: req.body.years,
        month_cos: req.body.month_cos,
        month_sin: req.body.month_sin,
        sunrise_hour: req.body.sunrise_hour
    };

    // Format the data to pass to the Python script
    const pythonScriptPath = 'path_to_your_script/predict_svm.py'; // Update path
    const args = Object.values(inputData).map(value => value.toString());

    // Run the Python script with the input data
    exec(`python ${pythonScriptPath} ${args.join(' ')}`, (error, stdout, stderr) => {
        if (error) {
        ...
        res.send(`<h2>Prediction Result: ${stdout.trim()}</h2>`);
    });
});
